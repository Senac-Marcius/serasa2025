...
export default function CollectionViewScreen() {
  ...
  useEffect(() => {
    async function getTodos() {
      ...
      if (retorno.status && retorno.data && retorno.data.length > 0) {
        setItems(retorno.data); // Armazena todos os itens
      }
      setLoading(false);
    }
    getTodos();
  ...
  const uniqueTypologies = Array.from(new Set(items.map(item => item.typology)));
  const uniqueYears = Array.from(new Set(items.map(item => item.year)));
  const uniqueLanguages = Array.from(new Set(items.map(item => item.language)));
  const uniqueResponsible = Array.from(new Set(items.map(item => item.responsible)));
  const uniqueSubject = Array.from(new Set(items.map(item => item.subject)));
